...
def main() -> int:
    ...
    assert compacted_total <= original_total, "compacted must not exceed original"
    snap_kept_recent = 0
    snap_trimmed_old = 0
    val_kept_recent = 0
    val_trimmed_old = 0
    ...
    assert snap_kept_recent >= 18, f"want >=18 recent snapshots kept, got {snap_kept_recent}"
    assert snap_trimmed_old <= 2, f"want <=2 old snapshots trimmed, got {snap_trimmed_old}"
