...
``thesaurus_id -> ThesauriSnapshot`` map, and the per-field
value counters from ``metadata.db``. Bundling them into a
single object keeps the match driver a thin flow.
"""
...
class MatchContextLoader:
    """Load the mapping, template, thesauri, and field counters for the match driver."""
