...
:mod:`browser_agent.drivers.step_5_upload_to_uwazi`. The driver takes
...
hand before running ``step_5_upload_to_uwazi.py``.
"""
...
class MatchDriver:
    ...
    def _query_metadata_rows(self) -> list[tuple[str, str, str]]:
        """Return ``(core_id, core_task_slug, data_json)`` rows from the run's metadata.db."""
    ...
    def _print_metadata_summary(self) -> None:
        ...
        for core_id, core_task_slug, data_json in self._query_metadata_rows():
            print(f"  {core_id} ({core_task_slug}): {_extract_field_names(data_json)}")
