...
contract — they MUST NOT import from this project. When a task extracts
metadata from many pages, the script needs to persist each record to a
shared SQLite store so downstream scripts can query it. This helper is
shipped as a plain-Python string and prepended to every emitted
``python_code``, mirroring the :mod:`emitted_page_wait` pattern.
...
(no duplicates). The DB path and task slug are computed from ``__file__``
at runtime — no injection needed, keeping the block a plain string literal.
"""
